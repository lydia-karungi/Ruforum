    ...
    if request.method == 'GET':
        ...
        if request.user.role == 'ADMIN' or request.user.is_superuser:
            ...
            tasks = Task.objects.filter(
                Q(created_by=request.user) or Q(assigned_to=request.user)).distinct()
    ...
    def form_valid(self, form):
        ...
        if self.request.is_ajax():
            data = {'success_url': reverse_lazy(
                'tasks:tasks_list'), 'error': False}
            return JsonResponse(data)
        messages.add_message(self.request, messages.SUCCESS, 'Creaded New Task successfully.')
...
def task_detail(request, task_id):
    ...
    if request.method == 'GET':
        ...
        if request.user.is_superuser or request.user.role == 'ADMIN':
            users_mention = list(User.objects.all().values('business_email'))
        elif request.user != task.created_by:
            users_mention = [{'business_email': task.created_by.business_email}]
        else:
            users_mention = list(task.assigned_to.all().values('user__business_email'))
...
def task_edit(request, task_id):
    ...
    if request.method == 'GET':
        if request.user.role == 'ADMIN' or request.user.is_superuser:
            users = User.objects.filter(is_active=True).order_by('business_email')
        else:
            users = User.objects.filter(groups__name__in=['Administrative/HR', 'Staff']).order_by('first_name', 'last_name')
